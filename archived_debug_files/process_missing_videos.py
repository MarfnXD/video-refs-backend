"""
Script para processar bookmarks sem cloud_video_url
Baixa vídeos via Apify, faz upload para cloud e gera thumbnails
"""
import os
import sys
...
async def process_bookmark(bookmark_id: str, url: str, user_id: str, title: str) -> bool:
    ...
    try:
        ...
        logger.info(f"✅ SUCESSO! Vídeo e thumbnail salvos")
        return True
...
async def main():
    # Buscar bookmarks sem cloud_video_url
    result = supabase.table('bookmarks').select('id, smart_title, url, user_id').is_('cloud_video_url', 'null').limit(10).execute()

    bookmarks = result.data

    logger.info("="*80)
    logger.info("🚀 Processamento de Bookmarks sem cloud_video_url")
