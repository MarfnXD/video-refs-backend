"""
Script para migração de TESTE - 3 vídeos apenas
...
LIMIT = 10  # ⚠️ TESTE: apenas 10 vídeos
...
print(f"✓ Encontradas {len(urls_to_migrate)} URLs válidas para migrar (máx {LIMIT})")
print()

# Mostrar URLs que serão migradas
print("📋 URLs que serão migradas:")
for item in urls_to_migrate:
    print(f"  ID {item['id']}: {item['url']}")
print()

# Confirmação
confirm = input("⚠️  Continuar com a migração? (s/n): ")
if confirm.lower() != 's':
...
print()
print("="*80)
print(f"MIGRAÇÃO TESTE - {len(urls_to_migrate)} VÍDEOS")
print("="*80)
print()
...
print()
print("="*80)
print(f"✅ MIGRAÇÃO TESTE INICIADA")
print("="*80)
print()
